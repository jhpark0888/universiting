...
class StatusController extends GetxController {
  ...
  void makeAllReceiveList() {
    ...
    for (var alarmreceive in receiveList) {
      ...
      receiveHostprofileImage.value = getHostsList(alarmreceive.content);
    }
  }
  ...
  void makeAllSendList() {
    ...
    for (var alarmSend in sendList) {
      sendHostprofileImage.value = getHostsList(alarmSend.room);
      sendJoinMemberprofileImage.value =
          getJoinMemberList(alarmSend.joinmember);
  ...
  List<ProfileImageWidget> getJoinMemberList(List<Host> hosts) {
    List<ProfileImageWidget> imageList = <ProfileImageWidget>[];
    for (Host host in hosts) {
      imageList.add(ProfileImageWidget(host: host));
    }
    return imageList;
  }
}
